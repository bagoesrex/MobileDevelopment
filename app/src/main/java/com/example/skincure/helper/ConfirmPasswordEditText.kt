...
class ConfirmPasswordEditText @JvmOverloads constructor(
    ...
    init {
        ...
        addValidation { password ->
            when {
                password.isEmpty() -> context.getString(R.string.password_minimum_length)
                else -> null
            }
        }
    }
    ...
    fun isValid(): Boolean {
        ...
        return when {
            input != passwordToMatch -> {
                edError(context.getString(R.string.password_mismatch))
                false
            }
